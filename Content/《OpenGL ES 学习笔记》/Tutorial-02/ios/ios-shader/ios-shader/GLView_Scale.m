...
- (instancetype)initWithFrame:(CGRect)frame context:(EAGLContext *)context {
    ...
    if (self) {
        ...
        // 绘制：默认旋转 0.5 度;
        (void)(_fX = 0.5), (void)(_fY = 0.5), _fZ = 0.5;
...
- (void)loadShaders {
    // 读取 Shader 文件路径;
    NSString *vertShaderFile = [[NSBundle mainBundle] pathForResource:@"display-scale" ofType:@"vsh"];
    NSString *fragShaderFile = [[NSBundle mainBundle] pathForResource:@"display-scale" ofType:@"fsh"];
...
- (void)processRender:(float)fX y:(float)fY z:(float)fZ {
    ...
    [self processScale:fX scaleY:fY scaleZ:fZ];
...
- (void)processScale:(float)fX scaleY:(float)fY scaleZ:(float)fZ{
    GLuint scale = glGetUniformLocation(self.displayProgram, "scaleMatrix"); /*获取Shader里面的旋转矩阵*/
    ...
    glUniformMatrix4fv(scale, 1, GL_FALSE, (GLfloat *)&scaleMatrix[0]);
}
