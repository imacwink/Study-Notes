...
@interface GL3DView_Camera() <CustomButtonDelegate> {
...
@implementation GL3DView_Camera
...
- (void)processRenderCamera {
    ...
    glUniformMatrix4fv(glGetUniformLocation(self.displayProgram, "projectionMatrix"), 1, GL_FALSE, glm::value_ptr(projection));
    ...
    for(unsigned int i = 0; i < 10; i++) {
        glm::mat4 model;
        model = glm::translate(model, offsetPos[i]);
        float angle = 20.0f * i;
